...
abstract class RDD[T: ClassTag](
   ...
   */
  //每一种RDD都会有这个compute方法，例如MapPartitionsRDD，就是针对RDD中的某一个Partition执行我们给这个RDD定义的函数
  @DeveloperApi
  ...
  * 首先通过compute执行到该RDD的iterator方法，会发现storageLevel != StorageLevel.NONE
  * 那么就会通过CacheManager去获取数据，此时会发现通过CacheManager获取不到数据（因为是第一次执行）
  * 那么就会第一次计算该RDD产生的数据，然后通过CacheManager的putInBlockManager将其通过BlockManager进行持久化
  *
  * rdd所在的job运行结束了，然后启动单独的job进行checkpoint操作，此时又会执行到该RDD的iterator方法，
  * 此时还是会发现storageLevel != StorageLevel.NONE，但是此时通过CacheManager获取数据就不为空了，就会直接从磁盘缓存读取文件
  * 写入hdfs，进行checkpoint
  *
  * 这样以后再需要改RDD的数据的时候，首先从缓存读取，如果缓存中没有的话，那么就去checkpoint中读取
  * */
