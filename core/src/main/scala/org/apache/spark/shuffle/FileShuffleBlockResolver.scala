...
private[spark] class FileShuffleBlockResolver(conf: SparkConf)
   ...
   */
  //为指定的map task生成一个ShuffleWriterGroup
  def forMapTask(shuffleId: Int, mapId: Int, numBuckets: Int, serializer: Serializer,
    ...
    new ShuffleWriterGroup {
      ...
      //判断是否开启了consolidate机制，如果开启了这个机制，那么就不会为每一个map对应的每一个reduce生成一个bucket
      //而是在同一个组中的ShuffleMapTask共用一个bucket
      //实际上对于每一个buclet并不是直接获取一个ShuffleBlockFile的Writer而是获取一个ShufflefileGroup的writer
      //这样就实现了多个ShuffleMapTask的输出数据的合并
      val writers: Array[DiskBlockObjectWriter] = if (consolidateShuffleFiles) {
        ...
        //如果没有开启consolidate机制，也就是普通的shuffle操作的话
        Array.tabulate[DiskBlockObjectWriter](numBuckets) { bucketId =>
          //同样生成一个BlockID
          val blockId = ShuffleBlockId(shuffleId, mapId, bucketId)
