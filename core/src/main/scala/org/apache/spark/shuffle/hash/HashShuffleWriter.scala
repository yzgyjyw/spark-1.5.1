...
private[spark] class HashShuffleWriter[K, V](
  ...
  override def write(records: Iterator[Product2[K, V]]): Unit = {
    ...
    //首先需要判断是否需要在map端进行聚合
    //如果是reduceBykey这类操作的话，dep.mapSideCombine和dep.aggregator.isDefined
    //都为true
    //此时就会进行map端的聚合
    //例如是(hello,1)(hello,1)就会聚合成(hello,2)
    ...
    //遍历数据，对每一个数据 调用partitioner默认是HashPartitioner，生成bucketId
    //也就决定了每一个数据需要写入到哪一个bucket中
    for (elem <- iter) {
      val bucketId = dep.partitioner.getPartition(elem._1)
      //会调用ShuffleBlockManager.forMapTask()方法，来生成bucketId对应的Writer
      //然后将数据写入bucket
