  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    private def makeOffers() {
      ...
      /*上面代码中的executorDataMap，在客户的向Master注册Application的时候，
      Master已经为Application分配并启动好Executor，
      然后注册给CoarseGrainedSchedulerBackend，注册信息就是存储在executorDataMap数据结构中。*/
      val activeExecutors = executorDataMap.filterKeys(!executorsPendingToRemove.contains(_))

      //当前Applications所有的可用的executor，并将每一个executor封装成WorkerOffer对象
      // WorkerOffer对象中包含了executor中空闲的cores的数量
      val workOffers = activeExecutors.map { case (id, executorData) =>
        new WorkerOffer(id, executorData.executorHost, executorData.freeCores)
      }.toSeq

      //首先，调用TaskSchedulerImpl的resourceOffers方法，执行任务分配算法，将task分配到executors上去
      //然后，在tasks分配好后，执行launchTasks方法，将分配好的task封装到发送的launchTask消息中，
      // 并将这个消息发送到对应的executor上去，由executor启动并执行task
      //发送这个消息的当前的对象是DriverEndpoint
    ...
    private def makeOffers(executorId: String) {
      ...
      if (!executorsPendingToRemove.contains(executorId)) {
        ...
        //调用TaskSchedulerImpl的resourceOffers进行任务资源的分配
        launchTasks(scheduler.resourceOffers(workOffers))
      }
    }
    ...
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      for (task <- tasks.flatten) {
        ...
        if (serializedTask.limit >= akkaFrameSize - AkkaUtils.reservedSizeBytes) {
          ...
          //将executor的cores重新计算赋值
          executorData.freeCores -= scheduler.CPUS_PER_TASK
          //向executor发送launchTask的消息，启动task
