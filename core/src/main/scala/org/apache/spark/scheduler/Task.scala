...
private[spark] abstract class Task[T](
  ...
  : (T, AccumulatorUpdates) = {
    ...
    //比如说task重试了几次，task属于哪一个stage，task要处理的是RDD的哪一个partition
    ...
    try {
      //调用抽象方法runTask，意味着这个类只是一个抽象类，仅仅封装一些子类通用的操作和数据
      //Task的实现主要有ShuffleMapTask和ResultTask对应于ShuffleMapStage和ResultStage
