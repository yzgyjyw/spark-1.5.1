 ...
 */
//BlockManager运行在每一个节点上，包括driver和executor上，提供了再远程或者本地存取数据的功能
//支持内存，磁盘存储
private[spark] class BlockManager(
  ...
  //每一个BlockManager自己维护一个map
  //是blockid到blockInfo的映射
  //每个BlockInfo可以代表一个Block
  ...
  def initialize(appId: String): Unit = {
    ...
    //使用BlockTransferService针对远程数据传输
    blockTransferService.init(this)
    shuffleClient.init(appId)

    //为当前的BlockManager创建一个唯一的BlockManagerID
    //使用到了executorId，因为每一个BlockManager都关联到一个executor，BlockTransferService的port和host
    //从中我们可以看出一个block是通过一个节点上的executor唯一标识的
    ...
    //使用BlockManagerMasterEndpoint对当前的BlockManager进行注册
    //发送消息到BlockManagerMaster上去
  ...
  * */
  ...
  private def doGetRemote(blockId: BlockId, asBlockResult: Boolean): Option[Any] = {
    ...
    //BlockManagerMaster上，获取每一个blockId对应的BlockManager的信息
    //然后随机打乱
    val locations = Random.shuffle(master.getLocations(blockId))
    //遍历每一个BlockManager
    for (loc <- locations) {
      logDebug(s"Getting remote block $blockId from $loc")
      //使用blockTransferService将远程的数据传输回来
      //连接的时候使用的blockManager的唯一标识就是host，port和exectuorId
  ...
  private def doPut(
    ...
    //为要写入的block创建一个blockInfo，并放入blockInfoMap中
    val putBlockInfo = {
    ...
    putBlockInfo.synchronized {
      ...
      try {
        ...
        if (putBlockStatus.storageLevel != StorageLevel.NONE) {
          ...
          if (tellMaster) {
            //将写入的block数据发送到BlockManagerMasterEndPoint中，以便于进行block数据的同步和维护
