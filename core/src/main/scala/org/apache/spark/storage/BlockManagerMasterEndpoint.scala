...
class BlockManagerMasterEndpoint(
  ...
  //BlockManagerMaster需要维护每一个BlockManager的BlockManagerInfo
  private val blockManagerInfo = new mutable.HashMap[BlockManagerId, BlockManagerInfo]

  // Mapping from executor ID to block manager ID.
  //executorId--->managerId
  //也就是说每一个executor是与一个BlockManager相关联的
  ...
  private def register(id: BlockManagerId, maxMemSize: Long, slaveEndpoint: RpcEndpointRef) {
    ...
    if (!blockManagerInfo.contains(id)) {
      //这里其实是做一个同步，如果blockManagerInfo（BlockManagerId-->BlockManagerInfo）
      // 这个Map里查不到这个block，那么blockManagerIdByExecutor(executorId--->managerId)
      //这个map里面也不应该有这个blockManager，此时如果有的话就应该清除
  ...
  private def updateBlockInfo(
    ...
    //blockLocations map:blockId--->HashSet(BlockManagerID)
    //每一个Block可能会在多个BlockManager上面
    //例如当StorageLevel设置成_2这种的，就需要将block replication设置为2，放到其他的BlockManager上
    //所以这边会保存locations的信息
...
//每一个BlockManager都有一个blockManagerInfo,相当于BlockManager的元数据信息
