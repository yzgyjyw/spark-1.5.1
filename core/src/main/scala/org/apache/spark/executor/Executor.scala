...
private[spark] class Executor(
  ...
  def launchTask(
    ...
    //对于每一个task都会创建一个TaskRunner，TaskRunner实现的是Runnable接口
    val tr = new TaskRunner(context, taskId = taskId, attemptNumber = attemptNumber, taskName,
      serializedTask)

    runningTasks.put(taskId, tr)

    //将线程丢入线程池进行，线程池内部是需要排队的
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      //生成内存管理对象taskMemoryManager，用于任务运行期间的内存管理
      ...
      try {
        ...
        val (value, accumUpdates) = try {
          //执行task
          //这里的res其实就是MapStatus，封装了ShuffleMapTask计算的数据，输出的位置
          //后面如果还是一个ShuffleMapTask，就会联系MapOutputTracker,来获取上一个ShuffleMapTask的输出的位置，然后通过网络拉取数据
          //ResultTask也是一样的
        ...
        //对MapStatus进行了各种序列化，因为后面需要将其发送到Driver通过网络
        ...
        val serializedResult: ByteBuffer = {
          ...
          //akkaFrameSize:akka框架传输的最大值
            //AkkaUtils.reservedSizeBytes:akka的预留空间
            //这两个值的差值作为阈值
            //计算结果大于上面的akkaFrameSize - AkkaUtils.reservedSizeBytes
          // 会把结果以taskId为编号存入到BlockManager中，然后把该编号发送到Driver端
        ...
        //task计算完毕后向Driver发送一个StatusUpdate消息
