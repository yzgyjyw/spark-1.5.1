...
private[deploy] class Master(
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case RegisterApplication(description, driver) => {
      ...
      if (state == RecoveryState.STANDBY) {
        ...
        //使用传递过来的description构建ApplicationInfo
        val app = createApplication(description, driver)
        //执行ApplicationInfo的注册
        /*
        * 1.将ApplicationInfo加载到内存中的队列中
        * 2.将ApplicationInfo加载到等待队列中
        * */
        registerApplication(app)
        logInfo("Registered app " + description.name + " with ID " + app.id)
        //将构建好的ApplicationInfo持久化
    ...
    case ExecutorStateChanged(appId, execId, state, message, exitStatus) => {
      //根据appId先获取到Application，然后再从中根据execId获取到executors
    ...
    case DriverStateChanged(driverId, state, exception) => {
      state match {
          //只要Driver的状态变成了下面这几种，那么就移除Driver
  ...
  private def scheduleExecutorsOnWorkers(
    ...
    /**
      ...
      * 3.if 新建一个executor 判断worker上的内存是否足够，判断executor的数量是否达到了上限
      * 4.不是 新建一个executor 那么就只需要判断1和2
      */
    def canLaunchExecutor(pos: Int): Boolean = {
      //只要该APP还有没有分配的CPU，并且该cpu的数量要大于每一个Executor上允许分配的CPU的数量
      val keepScheduling = coresToAssign >= minCoresPerExecutor

      //判断该worker上是否还有足够的cores
      //worker上剩余的cores减去已经被分配出去的cores
      val enoughCores = usableWorkers(pos).coresFree - assignedCores(pos) >= minCoresPerExecutor

      // If we allow multiple executors per worker, then we can always launch new executors.
      // Otherwise, if there is already an executor on this worker, just give it more cores.
      //当允许在同一个worker上启动同一个app的多个executor，或者在该worker上还没有启动过该app的executor的时候
      //我们就会创建一个新的executor，否则我们就会只是在原来的exeutor上面增加他的cpuCore
      val launchingNewExecutor = !oneExecutorPerWorker || assignedExecutors(pos) == 0

      //在worker上新建一个Executor的情况
    ...
    //canLaunchExecutor无非就是判断worker上的cores和memory的数量是否满足条件
    ...
    while (freeWorkers.nonEmpty) {
      freeWorkers.foreach { pos =>
        ...
        while (keepScheduling && canLaunchExecutor(pos)) {
          ...
          /*
            * 假如我们只允许在一个worker上启动一个executor，那么设置该worker上被分配的executor的数量为1
            * 否则就是在原来的executors的数量上加上1
           */
          ...
          /*
          * 如果我们采用的是spreadOutApps这个算法，就意味着我们需要尽可能的将executor分配到足够多的
          * worker上，此时就应该设置keepScheduling设置为false，结束在该executors上的分配
          *
          * 如果我们采用的不是spreadOutApps这个算法，就意味着我们需要一直在这个worker上分配这个executor
          * 这个时候我们就需要设置keepScheduling为true，让其一直循环，一直到该worker上的资源不满足再继续分配
          * executor了
          * */
          if (spreadOutApps) {
      ...
      //因为最外层是一个while循环，所以这边在过滤一遍，如果分配结束了，那么canLaunchExecutor就会返回false
      //得到的freeworkers必然是一个空，最外层的循环就结束了
  ...
  private def startExecutorsOnWorkers(): Unit = {
    ...
    //对于每一个还没有完全分配好CPU的app进行Executor的分配
    for (app <- waitingApps if app.coresLeft > 0) {
      //每一个Executor需要分配的CPU的数量
      val coresPerExecutor: Option[Int] = app.desc.coresPerExecutor
      // Filter out workers that don't have enough resources to launch an executor
      //过滤掉那些没有足够的资源启动一个executor的worker
      ...
      //分配算法已经决定好了在哪个worker上分配多少cores，下面我们开始进行executor的分配
      for (pos <- 0 until usableWorkers.length if assignedCores(pos) > 0) {
        //为executor分配worker上的资源
  ...
  private def launchExecutor(worker: WorkerInfo, exec: ExecutorDesc): Unit = {
    ...
    //向worker端发送一个启动executor的消息
    worker.endpoint.send(LaunchExecutor(masterUrl,
      exec.application.id, exec.id, exec.application.desc, exec.cores, exec.memory))
    //向driver端发送一个ExecutorAdded消息
  ...
  private def registerWorker(worker: WorkerInfo): Boolean = {
    ...
    if (addressToWorker.contains(workerAddress)) {
      ...
      //如果发现这个worker已经被注册过，并且状态为unknown，那么就需要移除旧有的worker的信息
      //如果发现这个worker已经被注册过，并且状态不为unknown，那么就忽略这个worker，因为这个worker已经注册过了
  ...
  private def removeWorker(worker: WorkerInfo) {
    ...
    //移除worker中所有的executors
    for (exec <- worker.executors.values) {
          logInfo("Telling app of lost executor: " + exec.id)
          exec.application.driver.send(ExecutorUpdated(
          exec.id, ExecutorState.LOST, Some("worker lost"), None))
          //让worker所在的application移除该executor
          exec.application.removeExecutor(exec)
          }
          //移除该worker中所有的drivers
  ...
  private def registerApplication(app: ApplicationInfo): Unit = {
    ...
    //将ApplicationInfo加载到内存中的队列中
    apps += app
    idToApp(app.id) = app
    endpointToApp(app.driver) = app
    addressToApp(appAddress) = app
    //将ApplicationInfo加入到等待队列中
    waitingApps += app
  }
  ...
  private def launchDriver(worker: WorkerInfo, driver: DriverInfo) {
    ...
    driver.worker = Some(worker)
    //向worker的endpoint发送LaunchDriver的样例类，在spark1.5.1中
    //endpoint的实现只有Akka的实现还没有netty版本的实现
